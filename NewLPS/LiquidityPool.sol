...
contract LiquidityPool {
    ...
    uint256 public collectedFeesB;
       event addLiquidityEvent(uint amountA,uint amountB,address poolProvider);
    ...
    function withdrawFees() public onlyOwner {
        if (collectedFeesA > 0) {
            require(tokenA.transfer(owner, collectedFeesA), "failed");
            collectedFeesA = 0;
        }
        if (collectedFeesB > 0) {
            require(tokenB.transfer(owner, collectedFeesB), " failed");
            collectedFeesB = 0;
        }
    }
        ...
        returns (uint256 liquidity)
    {
        ...
        reserveA += amountA;
        reserveB += amountB;
        emit addLiquidityEvent(amountA, amountB, msg.sender);
    }
    ...
    // Swap A to B
    function swapAtoB(uint256 amountIn, uint256 expectSlippage)
        public
        returns (uint256)
    {
        ...
        require(
            expectSlippage > checkSlippage(amountInAfterFee),
            "actual slippage is too high"
        );
    ...
    // Swap B to A
    function swapBtoA(uint256 amountIn, uint256 expectSlippage)
        public
        returns (uint256)
    {
        require(amountIn > 0, "Invalid input amount");
        ...
        uint256 amountInAfterFee = amountIn - fee;
        require(
            expectSlippage > checkSlippageB(amountInAfterFee),
            "actual slippage is too high"
        );
    ...
    function getExpectedTokenB(uint256 amountIn)
        public
        view
    ...
    function getExpectedTokenA(uint256 amountIn)
        public
        view
        returns (uint256 expectedOut)
    {
